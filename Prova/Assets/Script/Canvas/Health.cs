...
public class Health : MonoBehaviour
{
    ...
    void Change(){
        ...
        if(player_life%2==0){
           Options[(int)Mathf.Floor(((float) player_life)/2)].GetComponent<RawImage>().enabled=false;                 
        }else{
    ...
    public void addHeart(){
        int player_life=GameManager.getInstance().getPlayer().getLife();
         if(player_life%2==1){
            Options[(int)Mathf.Floor(((float) player_life)/2)].GetComponent<RawImage>().enabled=true;             
        }else{
            Options[(int)Mathf.Floor(((float) player_life)/2)-1].GetComponent<RawImage>().texture=raw.GetComponent<RawImage>().texture;
        }
    }
    void Start()
    {
